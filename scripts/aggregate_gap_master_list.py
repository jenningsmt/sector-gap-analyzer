...
CANDIDATE_TYPE_ORDER = {"bracketed_gap": 0, "backward_extrap": 1, "forward_extrap": 2}
...
class Row:
    __slots__ = ("sector_slug", "candidate_type", "system_name", "family",
                 "subsector", "mass_prefix", "number", "steps_from_edge",
...
def write_master_csv(out_path: Path, rows: list[Row]) -> None:
    with out_path.open("w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "mass_prefix", "number", "steps_from_edge", "spansh_edge_number",
            "source_file",
        ])
        for row in rows:
            writer.writerow([
                row.sector_slug, row.candidate_type, row.system_name, row.family,
                row.subsector, row.mass_prefix, row.number, row.steps_from_edge,
...
def write_master_md(out_path: Path, rows: list[Row]) -> None:
    ...
    with out_path.open("w", encoding="utf-8") as f:
        ...
        f.write("## Summary\n\n")
        f.write("| Sector | Bracketed Gap | Backward Extrap | Forward Extrap | Total |\n")
        f.write("|--------|--------------:|-----------------:|----------------:|------:|\n")
        for sector_slug in sorted(by_sector):
            ...
            f.write(f"| {sector_slug} | {bg} | {be} | {fe} | {bg + be + fe} |\n")
        f.write("\n---\n\n")
        ...
        for sector_slug in sorted(by_sector):
            ...
            for candidate_type in ("bracketed_gap", "backward_extrap", "forward_extrap"):
